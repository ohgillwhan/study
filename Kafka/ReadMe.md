...
Kafka는 pub-sub모델의 메세지 큐이고, 분산환경에 특화되어 설계되어 있다는 특징을 가짐으로써, 기존의 RabbitMQ와 같은 다른 메세지큐와의 성능 차이가 난다(훨씬 빠르게 처리한다).   
그 외에도 클러스터 구성, fail-over, replication와 같은 여러 가지 특징들을 가지고 있다.
## 메시징 모델?
메시징 모델은 크게 큐(queue) 모델과 발행-구독(publish-subscribe) 모델로 나뉜다
큐 모델은 메시지가 쌓여있는 큐로부터 메시지를 가져와서 consumer pool에 있는 consumer 중 하나에 메시지를 할당하는 방식이다  
...
가장 간단한 예제를 들어보자, 신문사에서 신문의 종류(topic)에 메세지를 쓴다.  
우리는 그 해당 신문을 구독한다.
## kafka의 기본 구성 요소와 동작
...
일반적으로 하드디스크는 메모리보다 훨씬 느리다.  
하지만 특정한 조건일 경우에는 하드디스크가 더 빠르다.  
메모리 랜덤읽기보다 디스크의 순차읽기가 훨씬 빠르다는것이다.  
...
메세지는 카프카의 해당 토픽에 쓰여진다.  
쓰는 과정도 시간이 소비된다.  
몇 천건의 메세지가 동시에 카프카에 쓰여진다고 생각해보자.  
그러면 하나의 파티션에 순차적으로 append될 텐데, 처리하는게 좀 버겁지 않나 ?  
그렇기 때문에 여러개의 파티션을 두어서 분산저장을 하는 것이다.  
그러면 쓰는 처리가 병렬로 처리될테니 시간이 그만큼 절약 하게 될 것 아닌가?
...
해당 consumer를 partition owner라고 부른다.  
...
이 말인 즉슨, 같은 그룹내의 어떤 컨슈머가 down된다면, 걔가 원래 파티션1에 대해서 소비를 맡았던 놈인데, 파티션1에 대한 소비를 할 수 없는 상황이다.  
...
broker.id=1..n으로 함으로써 동일한 노드내에서 여러개의 broker서버를 띄울 수도 있다.  
